...
impl Engine<'_> {
    pub fn render(&mut self, buffer: &mut [u8], _width: usize) {
        ...
        unsafe {
            ...
            zoffset = 0.0; //3.0 + 6.0 * (0.1 * N as f32).sin();
            xoffset = 0.0; //3.0 + 6.0 * (0.1 * N as f32).cos();
        }
        ...
        for y_inv in 0..HEIGHT {
            for x in 0..WIDTH {
                ...
                let colour_scaled = colour_srgb.map(|i| (255.0 * i).round().clamp(0.0, 255.0));

                buffer[i * 4 + 0] = colour_scaled[2] as u8;
                buffer[i * 4 + 1] = colour_scaled[1] as u8;
                buffer[i * 4 + 2] = colour_scaled[0] as u8;
                //buffer[i * 4 + 3] = 0;
            }
        }
    }
